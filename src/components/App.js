...
export default props => (
  <Wrapper>
    <Controls>
      <Button onClick={props.zoomIn} disabled={props.zoomWidth > 170}>
        {'Zoom +'}
      </Button>
      <Button onClick={props.zoomOut} disabled={props.zoomWidth < 30}>
        {'Zoom -'}
      </Button>
    </Controls>
    <GridWrapper>
      <GridLines zoomWidth={props.zoomWidth} col={props.timeline.length}>
        {props.timeline.map(({ date, events }, idx) => (
          <Column key={date} col={idx + 1}>
            ...
            {Array(7)
              .fill()
              .map((_, i) => (
                <Content2 key={i} bg={idx % 2 === 0 ? '#FFF' : '#EEE'} row={i}>
                  <p />
                </Content2>
              ))}
          </Column>
      ...
      <Grid zoomWidth={props.zoomWidth} col={props.timeline.length}>
        {props.timeline.map(({ date, events }, idx) => (
          <Column key={date} col={idx + 1}>
            <Header>
